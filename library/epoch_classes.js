...
CalCell.prototype.onclick = function () 
{
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		if(owner.selectMultiple == true)  //if we can select multiple cells simultaneously, add the currently selected cell's date to the selectedDates array
		{
			...
			for(var i=0;i<tmp.length;i++)
			{
				month = tmp[i].getMonth() + 1;
				printMonth = (month < 10) ? "0" + month : month;
				date = tmp[i].getDate();
				printDate = (date < 10) ? "0" + date : date;
				formattedDate = tmp[i].getFullYear() + '-' + printMonth + '-' + printDate;
				dates.push(formattedDate);
			}
			var formattedDates = dates.join(',');
			var nodates = '';
			if (document.getElementById('nodates').checked) { 
				nodates = 'nodates=Yes&';
			}
			var thisyear = '';
			if (document.getElementById('thisyear').checked) { 
				thisyear = 'thisyear=Yes&';
			}
			parent.playlistWindow.location.href = "https://deliciousagony.com/.suite/library/getLibrary.php?" + nodates + thisyear + "date=" + formattedDates;
		}
		else //if we can only select one cell at a time
		{
			...
			printMonth = (month < 10) ? "0" + month : month;
			printDate = (cell.date.getDate() < 10) ? "0" + cell.date.getDate() : cell.date.getDate();
			formattedDate = cell.date.getFullYear() + '-' + printMonth + '-' + printDate;
			var nodates = '';
			if (document.getElementById('nodates').checked) { 
				nodates = 'nodates=Yes&';
			}
			...
			parent.playlistWindow.location.href = "https://deliciousagony.com/.suite/library/getLibrary.php?" + nodates + thisyear + "date=" + formattedDate;
...
CalCell.prototype.setClass = function ()  //private: sets the CSS class of the cell based on the specified criteria
{
	...
	var date = this.date;
	var thisdate = date.getFullYear() + '-' + date.getMonth() + '-' + date.getDate();
