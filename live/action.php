<?php
  ...
  function print_results ($argument, $showid = null) {
	...
	// if (array_key_exists($artist,$ids)) { $artist .= " *"; }
        ...
        $classalbum = preg_replace("/[^A-Za-z0-9]/", '', $album);
        $id = $row['id'];
...
<tr>
<td valign=top>
<input type=checkbox class=\"$classalbum\" name=\"songs[]\" value=\"$id\">
</td>
<td></td>
<td valign=top>
...
<a href=\"javascript:CheckMe('$classalbum');\">*</a>
</td>
<td></td>
...
<form method=post name="playlist" action=playlist.php>
<table>
<tr>
<td></td>
<td width=10></td>
ENDLINE;
      ...
      if ($showid) {
      	...
      	print '<td width=10></td><th>Live</th>';
      print <<<ENDLINE
</tr>
$html
</table>
...
<?php
  ...
  switch ($action) {
    case 'album':
      ...
      print_results("select * from $table where live is null and artist='$artist' and album='$album' order by track,song");
      break;
    case 'reorder':
      $type = trim($_GET['type']);
    ...
    case 'artist':
      ...
      print_results("select * from $table where live is null and artist='$artist' order by year,album,track,song");
      break;
    case 'show':
      ...
      switch($show) {
    	case 'Interviews':
      	    print_results("select * from $table where live is null and album='Delicious Agony Interviews' order by year,album,track,song");
      	    break;
    	case 'SOP':
	...
	$like_string = implode(' or ', $string);
        print_results("select * from $table where live is null and station_id = 1 and ($like_string) order by artist,song", 1);
      }
	...
	$like_string = implode(' or ', $string);
	$days = $now - ($days * $day);
        print_results("select *, ROUND((UNIX_TIMESTAMP(NOW()) - date) / $day) + 1 as days from $table where live is null and $yearquery date > $days and ($like_string) order by days,artist,album,track");
      }
	...
	$like_string = implode(' or ', $string);
        print_results("select * from $table where live is null and year = $year and ($like_string) order by artist,album,track");
      }
	...
	$like_string = implode(' or ', $string);
        print_results("select * from $table where live is null and ((year > $first_year) and (year < $last_year)) and ($like_string) order by artist,album,track");
      }
	...
	$like_string = implode(' or ', $string);
        print_results("select * from $table where live is null and $yearquery length = $length and ($like_string) order by artist,album,track");
      }
	...
	$like_string = implode(' or ', $string);
        print_results("select * from $table where live is null and $yearquery (song like '%$title%' or album like '%$title%') and ($like_string) order by artist,album,track");
      }
      elseif ($show != 'Interviews' && ! preg_match('/ IDs/', $show)) {
      	print_results("select * from $table where live is null and $yearquery (artist like '$artist%' or artist like 'the $artist%') order by artist,year,album,track,song");
      }
      break;
    case 'station_id':
	...
	$artists = array();
  	foreach ($songs as $song) {
    	    $query_string = "select artist from $table where live is null and id='$song'";
    ...
    case 'filename':
      ...
      switch (trim($_POST['type'])) {
        case 'bg':
          print_results("select * from $table where live is null and filename like '$string%' order by artist,year,album,track,song");
          break;
        case 'ct':
          print_results("select * from $table where live is null and filename like '%$string%' order by artist,year,album,track,song");
          break;
    ...
    case 'needs':
      ...
      switch($need) {
        case 'VOs':
    	    $result_id = mysqli_query($mysql, "select id from $table where live is null and ((length = 0) and (artist like '%voiceo%')) order by song");
    	    $row_num = mysqli_num_rows($result_id);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
                $id = $row['id'];
        	    break;
        ...
        break;
        case 'Donation':
            $result_id = mysqli_query($mysql, "select * from $table where live is null and song like 'Donation Promo%' and length < 48 and song not like '%Rick%' order by song");
            $row_num = mysqli_num_rows($result_id);
        ...
        break;
        case 'Random Promo':
            $result_id = mysqli_query($mysql, "select * from $table where live is null and song not like '%donation%' and song not like '%msk%' and song like '%promo%' and length < 120 order by song");
            $row_num = mysqli_num_rows($result_id);
            $count = rand(1, $row_num);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
                $rowcount++;
        ...
        break;
        case 'DA shorts':
            $result_id = mysqli_query($mysql, "select * from $table where live is null and song like 'Delicious Agony ID%' order by song");
            $row_num = mysqli_num_rows($result_id);
            $count = rand(1, $row_num);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
                $rowcount++;
        ...
        break;
        case 'Intro':
            $result_id = mysqli_query($mysql, "select id from $table where live is null and filename like '%DA sh%' order by song");
            $row_num = mysqli_num_rows($result_id);
            $count = rand(1, $row_num);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
                $rowcount++;
            ...
            $result_id = mysqli_query($mysql, "select id from $table where live is null and song = '$intro'");
            $row_num = mysqli_num_rows($result_id);
            $count = rand(1, $row_num);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
               $id .= "," . $row['id'];
        ...
        case 'Promo':
            ...
            $result_id = mysqli_query($mysql, "select id from $table where live is null and song = '$promo'");
            $row_num = mysqli_num_rows($result_id);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
               $id = $row['id'];
        	   break;
        ...
        case 'Donation Promo':
            ...
            $result_id = mysqli_query($mysql, "select id from $table where live is null and song = '$donation'");
            $row_num = mysqli_num_rows($result_id);
            while ($row = mysqli_fetch_array($result_id, MYSQLI_ASSOC)) {
               $id = $row['id'];
        	   break;
    ...
    case 'date':
      ...
      switch($string) {
    	case 'Holiday':
      	    print_results("select * from $table where live is null and filename like '%Holiday Music%' order by artist,album,track,song");
      	    break;
        case 'New':
          $new = trim($_POST['new']);
	  switch($new) {
	  ...
	  $newdays = $now - ($newdays * $day);
          print_results("select *, (ROUND((UNIX_TIMESTAMP(NOW()) - date) / $day) + 1) as days from $table where live is null and date > $newdays order by days,artist,album,track,song");
          break;
        case 'Days':
          $days = trim($_POST['days']);
	  $days = $now - ($days * $day);
          print_results("select *, (ROUND((UNIX_TIMESTAMP(NOW()) - date) / $day) + 1) as days from $table where live is null and date > $days order by days,artist,album,track,song");
          break;
        case 'Year':
          $year = trim($_POST['year']);
          print_results("select * from $table where live is null and year = $year order by artist,album,track,song");
          break;
        case 'Decade':
          $first_year = trim($_POST['decade']) - 1;
          $last_year = $first_year + 11;
          print_results("select * from $table where live is null and ((year > $first_year) and (year < $last_year)) order by year,artist,album,track,song");
          break;
      }
      break;
    case 'lastquery':
    ...
    case 'id':
      $id = trim($_POST['id']);
      print_results("select * from $table where live is null and id = '$id'");
      break;
    case 'title':
      $string = trim($_POST['submit']);
      switch($string) {
        case 'Song':
          ...
          switch (trim($_POST['songtype'])) {
            case 'bg':
              print_results("select * from $table where live is null and song like '$song%' order by song");
              break;
            case 'ct':
              print_results("select * from $table where live is null and song like '%$song%' order by song");
              break;
        ...
        case 'Artist':
          ...
          switch (trim($_POST['artisttype'])) {
            case 'bg':
              print_results("select * from $table where live is null and (artist like '$artist%' or artist like 'the $artist%') order by artist,year,album,track,song");
              break;
            case 'ct':
              print_results("select * from $table where live is null and artist like '%$artist%' order by artist,year,album,track,song");
              break;
            case 'ed':
              print_results("select * from $table where live is null and artist like '%$artist' order by artist,year,album,track,song");
              break;
        ...
        case 'Album':
          ...
          switch (trim($_POST['albumtype'])) {
            case 'bg':
              print_results("select * from $table where live is null and album like '$album%' order by artist,track,song");
              break;
            case 'ct':
              print_results("select * from $table where live is null and album like '%$album%' order by artist,track,song");
              break;
    ...
    case 'autos':
      ...
      if ($type == 'Intros') {
        print_results("select * from $table where live is null and artist = 'Delicious Agony' and song like '%intro' order by song");
        # print_results("select * from $table where live is null and song like '%intro' and song not like 'That year%'  and song != 'Intro' order by song");
      }
      break;
    case 'bitrate':
      ...
      switch (trim($_POST['type'])) {
        case 'lt':
          print_results("select * from $table where live is null and bitrate < $bitrate and filename not like '%idents%' order by bitrate desc,artist");
          break;
        case 'xt':
          print_results("select * from $table where live is null and bitrate = $bitrate and filename not like '%idents%' order by bitrate,artist");
          break;
        case 'gt':
          print_results("select * from $table where live is null and bitrate > $bitrate and filename not like '%idents%' order by bitrate,artist");
          break;
    ...
    case 'time':
      ...
      switch($string) {
        case 'Length':
          ...
          switch (trim($_POST['type'])) {
            case 'lt':
              print_results("select * from $table where live is null and length < $length and album not like '%Delicious Agony Interviews%' order by length desc");
              break;
            case 'xt':
              print_results("select * from $table where live is null and length = $length and album not like '%Delicious Agony Interviews%' order by length,artist");
              break;
            case 'gt':
              print_results("select * from $table where live is null and length > $length and album not like '%Delicious Agony Interviews%' order by length");
              break;
          }
          break;
        case 'Between':
          ...
          if ($len2 < $len1) {
            print_results("select * from $table where live is null and (length >= $len2 and length <= $len1)  order by length");
          } else {
            print_results("select * from $table where live is null and (length >= $len1 and length <= $len2)  order by length");
          }
          break;
    }
    break;
